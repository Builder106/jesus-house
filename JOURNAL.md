...
## 2026-07-05 — The A15 run found the fix the desktop never would: invisible layers must also be transparent #incident #milestone

The owed on-device run happened same-day, and it earned its keep. Setup re-taught the July 4 lesson twice over: the phone arrived with power saving ON (again), 60Hz-pinned (again), and 39.8° skin — and the first meter pass read a scary 29.5fps that was just Chrome cold-start plus first image decode (warm pass: 44.3). But 44 up-scroll wasn't parity either: the home page measured 70.1 on the same device minutes apart, and the no-art control put /rcf's ceiling at 81. The bisection was the beautiful kind — hide only the mark: 79.9; hide only the art: 80.1; both visible: 44.3. Each layer alone was free; together they halved the frame rate. The cause was the one portal behavior I hadn't ported: jh-portal-art fades to opacity 0 by the veil point, and RCF's art+mark stayed opacity 1 — two full-screen composited layers blending beneath an opaque cream veil for the entire held half of the pin, which is exactly the state up-scroll re-enters at. Holding the SCALE (the invisible-zoom fix) wasn't enough; the layers also had to become transparent. After adding the fade (opacity 1→0 over enter 0.36→0.46, mirroring the portal): up 79.0, down 79.0, p50 11.1ms (true 90Hz pacing), max frame 78ms, zero frames over 150ms — at the ceiling, on a 39.9° throttling device, and it happens to look better too: the flame now dissolves into the wash instead of being papered over. Also from the session: fpsmeter attaches by URL substring, and with both / and /rcf tabs open it grabbed the backgrounded one and hung forever on the scroll gesture (synthesized gestures stall on hidden tabs) — close the other tab first. The 4-home-test QA "failure" recurred under the full rig and passed 5/5 in isolation; twice now that's been machine contention, never code. Phone restored as found (power saving on). Parish confirmations remain the only open items.

## 2026-07-05 — RCF caught up with the homepage: perf parity + the missing cue #decision #milestone

With the homepage "complete," the owner chose two directions for /rcf (over content-confirmation and ship-as-is): a perf-parity pass and a design-consistency audit — the page had its June signature but never received July's mobile-perf campaign. All three proven fixes ported cleanly because the RCF hero is structurally the home portal: (1) the dusk SVG — carrying the same full-canvas feTurbulence grain that was the homepage's biggest single cost — became a 16KB 2880×1800 WebP (source in art-src/, and the simple case: no theme tokens, so `rsvg-convert` regenerates it without the build/browser round-trip); (2) the dive moved to a compositor-side scroll-driven animation (`--rcf-hero` view-timeline; `contain` progress IS the JS `--enter`, verified to matrix-level parity: 2.1 at enter 0.2, exact); (3) both art and mark now HOLD at 3.53 from enter 0.46 — the veil is fully opaque there, and the old formula kept scaling invisible layers to 6.5×. The audit found the hero had drifted from two portal conventions: no scroll-to-enter cue (added — the mark needed `margin-bottom` clearance so the cue never sits on the graduation cap; first attempt left a measured 2px overlap at 390×844 because the self-centered mark converts only ~half of added margin into visible gap) and no `aria-labelledby` on the pinned section (added). /rcf also finally got QA coverage — 5 scenarios in e2e/features/rcf.feature; all 14 now pass. One near-false-alarm worth recording: the first full-suite run failed 4 home tests on 60s timeouts, looking exactly like a regression from the global styles.css edit — but the failing scenario passed in 892ms in isolation; the "regression" was my own rig (ng build + http-server + a live DevTools session) starving the dev server. Same lesson as the WebKit freeze storm: rule out your own test method first. Still owed: an on-device A15 run (phone not connected today) and the parish confirmations (@rcfcampus handle, Saturday slot).
