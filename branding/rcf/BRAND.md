...
> parish. The through-line we chose is **green** — the sage flyer matured into
> the actual RCCG-seal green — which gives RCF its own lane while keeping it in
> the family.
...
## The mark — the official RCF logo
...
the site (the chat's 🤩✨ energy lives in the copy's warmth, not glyphs).

## Anti-patterns (what RCF is NOT)
...
Same as the parish: **no student names or photos** on the site. "Student leaders"
stays generic; the launch flyer's faces never ship. Any future "meet the team"
needs explicit consent (PHOTO SLOT until then).

## Source & open items

Studied 2026-06-12 from the two meeting flyers in
`WhatsApp Chat - Wesleyan RCF/` (Apr 28 + May 5, 2026) and the brainstorm. Facts
...
no longer a redraw); page accents warmed green → gold to match it; full lockup
added as the closing sign-off. Possible follow-up: the Saturday vignette's desk
lamp is still green — warm it to brass if full warm-consistency is wanted.
